...
\title{AI-based Damage Detection for Power Grid Equipment\\with SaaS-oriented Design\\Midterm Report}
...
Power equipment is prone to bird nests, oil leaks, and local overheating; traditional manual inspection is inefficient, risky, and hard to scale. The project aims to build a scalable SaaS inspection platform: multi-modal data (UAV RGB, infrared, sensors, and historical records) as input; AI for damage detection and identification; outputs including alerts, work orders, and visual analytics; SaaS design with unified auth, multi-tenant isolation, and model registry for shared detection capability and integration with alert centres and work-order systems. By midterm, Module A (bird nest) is an end-to-end PoC, Module B (oil leak) has training and deployment in place, and Module C (infrared high-temperature) is implemented and integrated in Flask.

\subsection{Research Content and Technical Approach}

Strategy: ``vertical slice first, then horizontal scale''---complete one module from data to deployment, then multi-module registry and unified scheduling. Technical route: (1) \textbf{Data:} bird nest COCO, oil leak VOC/YOLO; scripts produce train/val sets; augmentation and preprocessing in the training pipeline. (2) \textbf{Training:} bird nest Mask R-CNN instance segmentation, oil leak YOLOv8 object detection; unified AMP, learning-rate schedules, DataLoader tuning. (3) \textbf{Deployment:} Flask microservice with REST API, batch upload and inference; bird nest, oil leak, and infrared are all integrated. Bird nest uses Mask R-CNN for boxes and masks for work-order linkage; oil leak uses YOLOv8 for speed and box-level localization.

\subsection{Module Plan}

\begin{itemize}
  \item \textbf{Module A (bird nest):} PoC completed; validates full pipeline (data -- training -- deployment).
  \item \textbf{Module B (oil leak):} YOLOv8s trained on the CSDN dataset (train10-csdn) and integrated with Flask; best validation mAP50 $\approx$ 0.39, mAP50-95 $\approx$ 0.26; see Section~3 for details.
  \item \textbf{Module C (infrared high-temperature):} Implemented and integrated in \texttt{deploy/flask-service}; supports user-defined alarm temperature range, OCR-based high/low temperature scale reading, and watermark/legend suppression; see Section~3.
\end{itemize}
...
The above matches the Work Plan from the proposal defence: complete a single-module vertical slice (Module A), then horizontal scale (Track 1) and new modules (Track 2), then integration and thesis. If the proposal contains a finer week-by-week or milestone breakdown, that document should be treated as the authoritative schedule.

% Uncomment to include Gantt chart if available (align with proposal)
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\textwidth]{../../schedule/project_schedule_gantt_proposal.png}
%   \caption{Overall arrangement and schedule (may match proposal Gantt).}
% \end{figure}
...
\begin{figure}[H]
  ...
  \caption{Bird nest: train/validation loss; curves level off around epoch 20.}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../../experiments/visualizations/pr_curve_bbox_ap50.png}
  \caption{Bird nest: bbox PR curves (IoU 0.50/0.75/0.90).}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../../experiments/visualizations/iou_hist_bbox.png}
  \caption{Bird nest: bbox IoU distribution; right skew indicates good localization.}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../../experiments/visualizations/threshold_sweep_bbox.png}
  \caption{Score threshold vs.\ precision/recall/F1 (IoU$\geq$0.5); F1 is best around threshold 0.4--0.5.}
\end{figure}
...
train10-csdn ran for 123 epochs; best validation metrics occur around epochs 93--94: \textbf{precision(B)} $\approx$ 0.83, \textbf{recall(B)} $\approx$ 0.34, \textbf{mAP50(B)} $\approx$ 0.39, \textbf{mAP50-95(B)} $\approx$ 0.26. Training losses (box/cls/dfl) decrease and stabilize over epochs. Figures~\ref{fig:oil-pr}, \ref{fig:oil-f1} and \ref{fig:oil-cm} show the validation PR curve, F1 curve, and normalized confusion matrix respectively. The best weights (\texttt{best.pt}) from this run can be copied to \texttt{checkpoints/oil\_leak\_yolov8.pt} for the Flask inference service.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../../src/oil_leak/runs/detect/train10-csdn/BoxPR_curve.png}
  \caption{Oil leak (train10-csdn) validation precision--recall curve; IoU threshold 0.5.}
  \label{fig:oil-pr}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Oil leak (train10-csdn) validation F1 curve; F1 at different confidence thresholds.}
  \label{fig:oil-f1}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Oil leak (train10-csdn) validation normalized confusion matrix.}
  \label{fig:oil-cm}
\end{figure}
...
Dataset size is still modest (54 bird nest images, $\sim$333 merged oil leak); annotation quality and consistency have a strong impact. On the deployment side, Flask with multi-process can handle moderate concurrency; for higher load or distributed inference, a queue and worker pool would be needed. These will be addressed in later phases through data expansion and architecture iteration.
