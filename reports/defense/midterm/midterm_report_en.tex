...
\begin{itemize}
  ...
  \item \textbf{Module B (oil leak):} YOLOv8 training and data-preparation pipeline in place and wired into Flask; full training results and metrics to be added.
  \item \textbf{Module C (infrared high-temperature):} Implemented and integrated in deploy/flask-service; supports user-defined alarm temperature range, OCR-based high/low temperature scale reading, and watermark/legend suppression; see Section~3.
\end{itemize}
...
Oil leak data come from two sources: an original VOC-format dataset ($\sim$103 images with XML annotations) and YOLO-format data from sources such as CSDN. \texttt{prepare\_dataset.py} supports three modes: VOC only, CSDN YOLO only, or merged ($\sim$333 images, $\sim$8:2 train/val). The script converts VOC boxes to YOLO normalized format, uses a single class \texttt{oil\_leak}, and writes a dataset YAML (paths, class count, names) for YOLOv8. Merged mode increases scale and diversity for better generalization. If runs such as \texttt{runs/detect/train10-csdn} exist, \texttt{results.csv} and \texttt{results.png} can be used in the report and the best weights (e.g.\ \texttt{best.pt}) copied to \texttt{checkpoints/oil\_leak\_yolov8.pt} for Flask.

\subsubsection{Training Config}

YOLOv8 (default \texttt{yolov8s.pt}) is used for single-class detection. Main hyperparameters: 150 epochs, input 640, batch 8, AdamW, cosine LR, mosaic/mixup and other augmentations, RAM cache, fixed seed for reproducibility; patience 30 for early stopping. The script is \texttt{src/oil\_leak/train.py} and by default uses the merged-dataset YAML; \texttt{--data}, \texttt{--epochs}, \texttt{--model}, etc.\ can be set from the command line for ablations and comparisons.
