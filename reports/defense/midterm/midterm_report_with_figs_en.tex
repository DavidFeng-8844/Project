...
\begin{abstract}
This report is the midterm summary of the project ``AI-based Damage Detection for Power Grid Equipment with SaaS-oriented Design.'' Power equipment in operation faces typical defects such as bird nests, oil leaks, and local overheating; traditional manual inspection is costly, risky, and difficult to cover at scale. The project aims to build a scalable SaaS inspection platform in which multi-modal data (UAV RGB, infrared, sensors, and historical records) serve as input, AI performs damage detection and identification, and outputs include alerts, work orders, and visual analytics. By midterm, Module A (bird nest) has been completed as an end-to-end PoC, Module B (oil leak) has training and deployment in place, and Module C (infrared high-temperature) has been implemented and integrated in Flask with custom temperature range, OCR-based scale reading, and watermark/legend suppression. The report summarizes the technical approach, completed work, open issues, and next steps, and is supplemented with training curves, validation figures, and deployment visualizations.
\end{abstract}
...
The overall arrangement and schedule align with the proposal, using a phased ``two-track'' plan (see the proposal schedule and Gantt chart for reference). The phases are described below.

\subsection{Phase Breakdown}

Weeks 1--4 (completed) focused on the Module A (bird nest detection) PoC, including data preparation, Mask R-CNN training, engineering optimizations, and Flask microservice deployment, with the vertical slice from data to callable service validated. Weeks 5--10 cover Track 1, the SaaS platform build, which includes multi-tenant backend work, RBAC, task orchestration, the AI model registry API, and productizing PoC training utilities. Weeks 11--18 correspond to Track 2, the new module extension, where Module C (infrared) is already integrated, Module B (oil leak) will be completed with full training and metrics, platform data and training will be reused, and all three modules will be consolidated in the registry and microservice catalog. Weeks 19--20 are reserved for final integration and defence, including end-to-end testing for multi-module and multi-user scenarios, system optimization, final report writing, and defence preparation.

\subsection{Schedule Note}

The above matches the Work Plan from the proposal defence, which completes a single-module vertical slice (Module A), then horizontal scale (Track 1) and new modules (Track 2), then integration and thesis. If the proposal contains a finer week-by-week or milestone breakdown, that document should be treated as the authoritative schedule. This staged approach ensures that each track delivers testable outcomes before moving to the next, reducing integration risk at the final stage.
...
Oil leak data come from two sources, an original VOC-format dataset ($\sim$103 images with XML annotations) and YOLO-format data from sources such as CSDN. \texttt{prepare\_dataset.py} supports three modes including VOC only, CSDN YOLO only, or merged ($\sim$333 images, $\sim$8:2 train/val). The script converts VOC boxes to YOLO normalized format, uses a single class \texttt{oil\_leak}, and writes a dataset YAML for YOLOv8. The training results reported here are from \texttt{runs/detect/train10-csdn}, which uses the \textbf{CSDN dataset} (\texttt{oil\_leak\_csdn.yaml}), meaning YOLO-format data from the CSDN source only.

\subsubsection{Training Config}

YOLOv8s (\texttt{yolov8s.pt}) is used for single-class detection. Main hyperparameters for train10-csdn include 150 epochs, input 640, batch 8, AdamW, cosine LR, mosaic 0.3, mixup 0.05, RAM cache, patience 30, AMP enabled, and fixed random seed 42. The training script is \texttt{src/oil\_leak/train.py}, and \texttt{--data}, \texttt{--epochs}, \texttt{--model}, etc.\ can be set from the command line.

\subsubsection{Validation Metrics and Training Results}

train10-csdn ran for 123 epochs; best validation metrics occur around epochs 93--94, with \textbf{precision(B)} $\approx$ 0.83, \textbf{recall(B)} $\approx$ 0.34, \textbf{mAP50(B)} $\approx$ 0.39, and \textbf{mAP50-95(B)} $\approx$ 0.26. Training losses (box/cls/dfl) decrease and stabilize over epochs. Figure~\ref{fig:oil_leak_training} shows the validation PR curve, F1 curve, training results, and normalized confusion matrix. The best weights (\texttt{best.pt}) from this run can be copied to \texttt{checkpoints/oil\_leak\_yolov8.pt} for the Flask inference service.
...
Deployed under \texttt{deploy/flask-service}, the service supports batch image upload and on-demand inference. Users select a registered model (e.g.\ ``bird nest'', ``oil leak'', or ``infrared high-temperature'') and upload one or more images; the server runs the corresponding logic (Mask R-CNN, YOLOv8, or infrared hybrid detection) and returns JSON (boxes, confidence/temperature, class) along with visualization images with boxes and masks. The system provides single and ZIP download for offline review and archiving, statistics and dashboards for historical inference tasks and detections, and robust handling of large images via resize or tiling to avoid OOM. Exceptions are caught at the API layer with clear error messages for stability.

\subsubsection{API and Run Mode}

The request flow is that a user uploads images and selects a model via the web UI, the server resolves the weight path by model type and runs inference, and then returns structured results and a visualization URL. In development the service runs with \texttt{python run.py} at the default \texttt{0.0.0.0:5000}; in production it uses Gunicorn multi-worker (e.g.\ \texttt{gunicorn -w 4 wsgi:app}) for higher concurrency.
...
Main features include a user-defined alarm temperature range, OCR for the high/low temperature scale, and watermark and legend suppression. The front end or API passes an alarm temperature threshold (default 50$^\circ$C), the service maps pixel intensity to temperature, and it filters regions above the threshold. EasyOCR is used to detect and recognize text on the infrared image; the maximum and minimum temperatures corresponding to the image scale are parsed from the legend or scale area and used to map pixel intensity to temperature, with heuristic correction for common OCR errors. A border watermark mask detects and masks bright white watermark/logo regions near the image border so they are not treated as hotspots, and OCR text-region masks exclude legend and scale digits to reduce false positives. This combination allows reliable hotspot detection without training a dedicated deep-learning model on infrared data. Outputs include hotspot boxes, per-hotspot temperature, global max read temperature, max hotspot temperature, and an annotated visualization.
...
Track 1 focuses on a multi-tenant backend with RBAC and task orchestration, including data models and APIs for users/roles/permissions, a task queue (e.g.\ Celery or in-memory), and task-status queries so multiple users can submit detection tasks and view results concurrently. It also includes an AI model registry API for unified registration, query, enable/disable, and weight-path management so the frontend and scheduler can select by name or type and allow future versioning and A/B testing. Finally, it will productize PoC training utilities by making augmentation config, training logs, and checkpoint management from the current scripts configurable and reusable for the oil leak and infrared modules to avoid duplication.

\subsection{Track 2: New Modules (weeks 11--18)}

Track 2 completes Module B (oil leak) experiments and consolidates Module C (infrared) by finishing full training and validation metrics for oil leak on the merged dataset and updating the registry, while infrared is already deployed and can shift to robustness and multi-device infrared image adaptation. New modules should reuse \texttt{prepare\_dataset}, training scripts, and the deployment pipeline, only changing data paths and model type to reduce maintenance. Registration work adds new model entries in the database and frontend so task routing and inference logic correctly recognize and invoke them.

\subsection{Wrap-up and Defence (weeks 19--20)}

Final integration, system testing, and report writing include end-to-end tests for multi-module and multi-user scenarios, consolidation of experiment data and screenshots, and completion of the final report and defence materials. Expected deliverables are a demonstrable three-module (bird nest / oil leak / infrared) SaaS prototype and a written report and defence presentation that meet the requirements.
...
Current bird nest validation gives AP$_{50}$=1.0 and IoU $\approx$ 1.0 on a same-distribution validation set. In real complex conditions (haze, backlight, new tower types, different seasons and regions), performance may drop. Data augmentation, negative and hard-sample design, and multi-scene evaluation are therefore platform-wide needs, and the plan is to add cross-scene data or hold out field data not used in training for generalization tests. The same care will be applied to negative and hard samples in the oil leak and infrared modules, avoiding overfitting to small, same-distribution data at the cost of real-world robustness.

\subsection{Technical Choices}

For the SaaS backend, whether to prioritize a feature-complete prototype (e.g.\ simple RBAC, task queue, multi-tenant isolation) or invest in production-grade auth (OAuth2, audit logs) from day one depends on time and defence goals; the current preference is to deliver a demonstrable multi-model, multi-task prototype first, then add auth and auditing if time allows. For the oil leak module, YOLOv8 box-level detection is sufficient for ``presence and approximate location''; if the business later requires precise oil-boundary segmentation (e.g.\ area statistics, trend comparison), semantic segmentation (e.g.\ U-Net) or combination with instance segmentation can be considered, with pixel-level annotations. For the infrared module, high-temperature detection is already integrated as a standalone module with custom alarm temperature and OCR scale reading, and follow-up may consider fusion with RGB for multi-modal analysis (e.g.\ RGB + heatmap), depending on field data and operator workflow.
